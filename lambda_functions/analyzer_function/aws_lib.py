...
SNS_PUBLISH_SUBJECT_MAX_SIZE = 99 # maximum character limit for the subject field in (SNS) message
...
def _elide_string_middle(text, max_length):
    ...
    # Used because of the fixed size of the Subject in SNS
...
def put_metric_data(num_yara_rules, binaries):
    # Add custom metric data to CloudWatch.
    ...
    metric_data = [
        ...
        # maybe still add some more metrics
    ]
...
class DynamoMatchTable(object):
    ...
    def _most_recent_item(self, sha):
        # Query and retrieve the table for the most recent entry with the given SHA.
    ...
    def save_matches(self, binary, lambda_version):
        # Save YARA match results to the Dynamo table and return :
